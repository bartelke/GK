...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            ...
            vertex_color = vec4(0.2, 0.2, 0.8, 0.2);
        }
    """
...
def render(time):
    glClearBufferfv(GL_COLOR, 0, [1.0, 0.5, 0.2, 1.0])
    ...
    M_matrix = glm.translate(M_matrix, glm.vec3(2.5, 2.5, 0.0))
    glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))

    for i in range(5):
        M_matrix = glm.translate(M_matrix, glm.vec3(-1.0, -5.0, 0.0))
        glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))
